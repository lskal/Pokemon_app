...
export const BasketCard = ({
    ...
    setCart((prev) => {
      ...
      if (exists) {
        return prev.map((item) =>
          item.id === cart.id ? { ...item, quantity: item.quantity && item.quantity + 1 } : item,
        );
      }
  ...
  const handleCartDecrease = () => {
    setCart((prev) => {
      return prev
        .map((item) =>
          item.id === cart.id ? { ...item, quantity: (item.quantity ?? 1) - 1 } : item,
        )
      ...
      <div className="cart_details">
        ...
        <div className="buttons_quantity">
          ...
          <div className="buttons_container">
            <button className="plus" onClick={handleCartIncrease}>
              +
            </button>
            <div className="cart_quantity">{cart.quantity}</div>
            <button className="minus" onClick={handleCartDecrease}>
              -
            </button>
          </div>
        </div>
