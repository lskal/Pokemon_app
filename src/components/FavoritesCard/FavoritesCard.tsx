  ...
  return (
    <>
      <div className="favorites_card" key={favorite.id}>
        <div
          className="favorite_image_container"
          onClick={() => navigate(`/pokemon/${favorite.id}`)}
        >
          <div className="favorite_icon" onClick={handleFavIconClick}>
            {favorite.favorite ? <MdFavorite className="fav_icon_clicked" /> : <MdFavoriteBorder />}
          </div>
        ...
        <div className="favorite_title_container">
          {favorite.name && (
            <div className="favorite_title">
              {favorite.name.toUpperCase() || 'Title unavailable'}
            </div>
          )}
        </div>
      </div>
